...
<section class="admin-shell">
    ...
    <div class="admin-main">
        ...
        <section class="panel">
            ...
            <div class="panel__head headbar">
                <form method="get" action="<?= e(BASE_URL . 'index.php') ?>" class="filters-bar">
                        ...
                        <select name="office" class="pill-input">
                            ...
                            <?php foreach ($offices as $o): ?>
                                <option value="<?= (int) $o['office_id'] ?>"
                                    <?= (int) $filters['office'] === (int) $o['office_id'] ? 'selected' : '' ?>>
                                    <?= e($o['name']) ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    ...
                    <div class="seg">
                        ...
                        foreach ($statuses as $val => $label):
                            $id = 'st_' . ($val === '' ? 'all' : $val);
                            ?>
                            <input type="radio" id="<?= e($id) ?>" name="status" value="<?= e($val) ?>"
                                <?= $st === $val ? 'checked' : '' ?>>
                    ...
                    <div class="chips">
                        ...
                        <button class="chip" type="button" data-range="clear">Clear</button>
                    </div>
                    ...
                    <div class="actions">
                        ...
                        <?php if ($total): ?>
                            <a class="btn"
                                href="<?= e(BASE_URL . 'index.php?page=admin_appointments_export&' . http_build_query($filters)) ?>">Export
                                CSV</a>
                        <?php endif; ?>
                    </div>
                </form>
    ...
    <th style="width:8%">Actions</th> <!-- NEW -->
  </tr>
                        </thead>
                        <tbody>
                            <?php
  ...
  <form method="post" action="<?= e(BASE_URL.'index.php?page=admin_appointments') ?>"
    ...
    <input type="hidden" name="del"  value="<?= (int)$r['appointment_id'] ?>">
    <button class="icon-btn danger" title="Delete appointment" aria-label="Delete">
      <span class="icon-trash">🗑</span>
                ...
                <div class="pager">
                    <?php if ($page > 1): ?>
                        <a class="btn"
                            href="<?= e(BASE_URL . 'index.php?page=admin_appointments&' . http_build_query($filters + ['p' => $page - 1])) ?>">‹
                            Prev</a>
                    <?php endif; ?>
                    <span>Page <?= (int) $page ?> / <?= (int) $pages ?> &nbsp;•&nbsp; <?= (int) $total ?> total</span>
                    <?php if ($page < $pages): ?>
                        <a class="btn"
                            href="<?= e(BASE_URL . 'index.php?page=admin_appointments&' . http_build_query($filters + ['p' => $page + 1])) ?>">Next
                            ›</a>
                    <?php endif; ?>
                </div>
            <?php endif; ?>
...
<style>
    ...
    .tbl thead th {
        ...
        top: 0;
        z-index: 1;
        background: linear-gradient(#f7f9fc, #f4f7fb);
        text-align: left;
        font-weight: 600;
        color: #1f315c;
        padding: 10px;
...
.pill{
  ...
  border:1px solid #dbe2ea; border-radius:999px;
  box-shadow:0 1px 0 rgba(32,56,103,.03);
}
...
.seg{
  ...
  padding:6px; background:#fff; border:1px solid #dbe2ea; border-radius:999px;
}
.seg input{ position:absolute; opacity:0; pointer-events:none }
.seg label{
  padding:6px 10px; border-radius:999px; cursor:pointer;
  border:1px solid transparent; color:#274686; user-select:none;
}
.seg input:checked + label{
  background:#eaf1ff; border-color:#d6e6ff; color:#143a73;
  box-shadow:0 1px 0 rgba(32,56,103,.05) inset;
}

/* Quick range chips */
.chips{ display:flex; gap:6px; align-items:center }
.chip{
  padding:6px 10px; border-radius:999px; border:1px solid #dbe2ea;
  background:#fff; cursor:pointer; font-size:.92rem;
}
...
.tbl thead th{
  ...
  background:linear-gradient(#f7f9fc,#f4f7fb);
  text-align:left; font-weight:600; color:#1f315c;
  padding:10px; border-bottom:1px solid #e6ebf5
}
...
/* Per-office header row */
.office-row td{
  background:#f5f8ff; color:#1f315c; border-bottom:1px solid #e6ebf5;
  position:sticky; top:42px; z-index:0; padding:10px 10px
}
.off-name{ font-weight:700 }
.off-count{
  margin-left:8px; font-size:.86rem; color:#274686;
  padding:2px 8px; border-radius:999px; background:#eaf1ff; border:1px solid #d6e6ff
}
...
.icon-btn{
  ...
  width:32px; height:32px; border-radius:8px; border:1px solid #e6ebf5; background:#fff;
}
  ...
  form.querySelectorAll('.chip').forEach(ch => {
    ch.addEventListener('click', () => {
      ...
      if (kind === 'today'){
        ...
        const s = now, e = new Date(now); e.setDate(now.getDate()+7);
        from.value = fmt(s); to.value = fmt(e);
      } else if (kind === 'clear'){
        from.value = ''; to.value = '';
      }
